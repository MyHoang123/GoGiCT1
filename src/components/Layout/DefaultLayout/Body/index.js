...
import { getProduct } from './BodySlice';
...
// Ref
    ...
    async function AddHistoryUser(User) {
        try {
            await axios.post('https://severgogi.onrender.com/api/v12/addhistoryuser', User);
        } catch (error) {
    ...
    // const handleClickAddCard =  useCallback(async(button) => {
    //         cardButton.current[button].style.pointerEvents = 'auto'
    //         buttonFakeAddCard.current[button].classList.add('open')
    //         let x = 0
    //         let y = 0
    ...
    const handlePageChange = useCallback((selectedItem) => {
        ...
        dispatch(BodySlice.actions.changePage(selectedItem.selected + 1))
      },[])
