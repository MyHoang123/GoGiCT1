...
function App() {
    ...
    const {checkRegister, pass, passn, setPass, setPassn, phone, setPhone } = useContext(AuthContext)
    const handleChangeInput = useCallback((i,e) => {
    ...
    const handleClickVerifyOTP = useCallback( async(OTPsend,Phone) => {
            if(OTPsend.length === 6) {
                const OTPverify = {
                    Phone: Phone,
                    OTP: OTPsend
                ...
                try {
                    const response = await axios.post(`${process.env.REACT_APP_CALL_API}/api/v12/verifyotp`, OTPverify);
    ...
    const handleClickVerifyOTPRegister = useCallback( async(OTPsend,Phone) => {
            if(OTPsend.length === 6) {
                const OTPverify = {
                    Phone: Phone,
                    OTP: OTPsend
                ...
                try {
                    ...
                    if(response.data.massege === 'Thanh cong') {
                        ...
                        setStepRegister(2)
                        lineStatus.current.style.width = '170px'
    ...
    const handleClickPassword = useCallback((e) => {
    ...
    const handleClickRegister = useCallback(async (Pass,Passn,Phone,Gender) => {
        if(Pass === Passn) {
            const regexUpper = /^(?=.*[A-Z]).+$/;
            const regex = /^.{6,}$/;
            ...
            if(isValid && isValidUpper && Gender !== null) {
                ...
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: "Vui lòng đảm bản định dạng mật khẩu !",
    ...
    const handleClickLoginUser = useCallback((user,token) => {
            ...
            switch (navigationEntry.type) {
                case 'reload':
                   navigate('/loginuser')
                    break
                default:
                    console.log("Loại điều hướng không xác định.");
                    break;
            }
        }
    ...
    return ( 
        <div className={cx('Modal_authen_OTP')}>
            <div style={checkRegister ? {display:'flex'} : {display:'none'}} className={cx('Modal_authen_OTP_header_step')}>
            ...
            <div className={cx('Modal_authen_OTP_container')}>
                {stepRegister === 1 ? (
                               ...
                               <Link to='/loginuser'>
                                   <FontAwesomeIcon style={{fontSize:'20px',marginBottom:'0',color:'#990000'}} icon={faArrowLeft} />
                               </Link>
                               ...
                               {checkRegister ? (
                                   ...
                                   <button onClick={() => handleClickVerifyOTP(valueOTP,phone)}>Hoàng thành</button>
                               )}
                           </>
                ) : (stepRegister === 2 ? (
                            <>
                        ...
                        <div style={{marginBottom:'10px',padding:'0 80px'}} className={cx('Modal_login_input_list')}>
                            <input onChange={(e) => handleClickPassword(e)} style={{margin:'20px 0 0 0 '}} type='password' placeholder='Mật khẩu'/>
                                    <span ref={hightpass} className={cx('fake_pass')}></span>
                                    <ul className={cx('noti_password')} style={{fontSize:'12px'}}>
                    ...
                    <div ref={notiRegisterSuccess} className={cx('Modal_authen_OTP_container_success')}>
                        <h1>Thành Công</h1>
                        <FontAwesomeIcon style={{fontSize:'20px',color:'#2dc258'}} icon={faCircleCheckRegular} />
